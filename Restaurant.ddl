...
ALTER TABLE recipe ADD CONSTRAINT recipe_pk PRIMARY KEY ( dish_id,
                                                          step );
...
ALTER TABLE recipe
    ADD CONSTRAINT recipe_dishes_fk FOREIGN KEY ( dish_id )
        REFERENCES dishes ( id );
...
CREATE SEQUENCE orders_orders_id_seq START WITH 1 NOCACHE ORDER;

CREATE OR REPLACE TRIGGER orders_orders_id_trg BEFORE
    INSERT ON orders
    FOR EACH ROW
    WHEN ( new.orders_id IS NULL )
BEGIN
    :new.orders_id := orders_orders_id_seq.nextval;
END;
/
